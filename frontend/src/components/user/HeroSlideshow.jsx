...
export default function HeroSlideshow() {
	...
	const handleAnimationEnd = () => {
		const classes = ["slide-out-to-left",
			"slide-in-from-right",
			"slide-out-to-right",
			"slide-in-from-left"
		];
	...
	return (
		<div className="w-full flex">
			{/* Slide show section */}
			<div className="md:w-4/5 w-full relative overflow-hidden">
				<Slide ref={slideRef}
			...
			<div className="w-1/5 md:block hidden space-y-3 px-3">
				...
				</h1>
				{
					upNext.map(({ poster, id }) => {
						return (
							<Link to={'/movie/' + id}>
								<img key={id} src={poster} alt="" className="apect-video object-cover rounded" />
							</Link>
						)
					})
				}
	...
	return (
		<Link to={'/movie/' + id} ref={ref} className={"w-full cursor-pointer block " + className} {...rest} >
			{src ? <img
				...
				src={src} alt=""
			/> : null}
			{title ? <div className="absolute inset-0 flex flex-col justify-end py-3 bg-gradient-to-t
